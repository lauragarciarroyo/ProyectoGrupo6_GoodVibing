...
async function getMySpace(req, res, nex) {
    try {
        const { id } = req.params;
...
async function viewStories(req, res, next) {
    try{
        ...
        const story = await storiesRepository.findStoriesById(userId, storiesId);    
        res.send(story);
...
async function createStories(req, res, next) {
  ...
  async function editStories(req, res, next) {
    try {
      ...
      const updatedStory = await storiesRepository.updateStories(storiesId, userId);
...
module.exports = {homeStories, searchStories,viewStories, createStories, editStories, deleteStories, getMySpace}