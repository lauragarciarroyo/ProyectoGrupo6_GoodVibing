...
async function addWriteComments(dataComments, userId) {
    const query = 'INSERT INTO comments (user_id, stories_id, text) VALUES (?, ?, ?)';

    const [result] = await database.pool.query(query, [userId,data.storiesId,data.text]);
...
module.exports = {findComments, addWriteComments, editWriteComments, deleteComments}